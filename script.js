...
// --- Carousel for Gallery (optional, non-destructive) ---
try {
  const gSlides = document.getElementById('gallerySlides');
  const gPag = document.querySelector('#gallerySwiper .swiper-pagination');
  if (gSlides) {
    // Use the same list already loaded into galleryGrid
    const gal = await loadJSON('content/gallery.json');
    const list = (gal && gal.images) ? gal.images : [];
    gSlides.innerHTML = '';
    list.forEach(item => {
      const url = (item.image || item.src || item).toString();
      const slide = document.createElement('div');
      slide.className = 'swiper-slide';
      slide.innerHTML = `<img src="${url}" alt="portfolio" class="w-full h-auto">`;
    ...
    if (window.Swiper) {
      new Swiper('#gallerySwiper', {
        ...
        pagination: { el: gPag, clickable: true },
        autoplay: { delay: 2500, disableOnInteraction: false }
      });
    }
  }
} catch (err) { console.log('gallery carousel skipped', err); }
}
...
// --- Carousel for Student Work (optional) ---
try {
  const sSlides = document.getElementById('studentsSlides');
  const sPag = document.querySelector('#studentsSwiper .swiper-pagination');
  if (sSlides) {
    const data = await loadJSON('content/students.json');
    ...
    list.forEach(st => {
      const slide = document.createElement('div');
      slide.className = 'swiper-slide';
      slide.innerHTML = `<img src="${st.image}" alt="${st.name || 'student'}" class="w-full h-auto">`;
      sSlides.appendChild(slide);
    });
    if (window.Swiper) {
      new Swiper('#studentsSwiper', {
        ...
        spaceBetween: 12,
        breakpoints: { 640:{slidesPerView:2}, 1024:{slidesPerView:3} },
        pagination: { el: sPag, clickable: true },
        autoplay: { delay: 2500, disableOnInteraction: false }
      });
    }
  }
} catch (err) { console.log('students carousel skipped', err); }
            </div>
